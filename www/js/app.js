      ...
      if (window.localStorage.getItem('isConnected')) {
        ...
        if (null !== window.localStorage.getItem('accessToken')) {
          ...
          if (expire < Date.now()) {
            ...
            getter = "http://192.168.15.159/zenitth/zenitth-server/web/oauth/v2/token?client_id=" + token.clientId + "&client_secret=" + token.secretId + "&grant_type=refresh_token&refresh_token=" + accessToken.refreshToken;
          ...
          // Get access token
          expire = new Date().getTime() + (3600*1000);
          getter = "http://192.168.15.159/zenitth/zenitth-server/web/oauth/v2/token?grant_type=http://zenitth.com/grants/api_key&client_id=" + 
